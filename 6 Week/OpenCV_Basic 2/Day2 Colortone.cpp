...
Mat getCurveImage(const uchar(&curve)[256]);
...
int main()
{
	...
	Mat curve1_img = getCurveImage(curve1);
	Mat curve2_img = getCurveImage(curve2);
	...
	merge(channels, dst);
	imshow("src", src);
	imshow("dst [R -> curve1, B -> curve2]", dst);
	imshow("curve1", curve1_img);
	imshow("curve2", curve2_img);
	waitKey();
}
...
Mat getCurveImage(const uchar (&curve)[256])
{
	...
	Mat imgCurve(256, 256, CV_8UC1, Scalar(255));

	if (size != 256) {
		cout << "Not correct size : " << size << endl;
		return imgCurve;
	}

	for (int i = 0; i < 256; i++)
	{
		circle(imgCurve, Point(i, curve[i]), 2, Scalar(0), -1);
	}
	return imgCurve;
}